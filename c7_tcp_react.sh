...
if [ -f /etc/alpine-release ]; then
  ...
  echo "Alpine Linux 不支持 TCP React 模块，建议使用标准 BBR"
  echo "请运行: tcp.sh 或 tcpx.sh 来配置网络优化"
  exit 1
elif [ -f /etc/redhat-release ]; then
  OS_TYPE="centos"
elif [ -f /etc/debian_version ]; then
  OS_TYPE="debian"
  echo "此脚本专为 CentOS 7 设计，Debian/Ubuntu 系统请使用其他脚本"
  exit 1
else
  echo "不支持的操作系统"
  exit 1
...
if [ ${kernel_version} = "5.9.6" ]; then
	...
	sysctl -w net.ipv4.tcp_congestion_control=react_rc2
		...
		yum -y install devtoolset-8-gcc make && source /opt/rh/devtoolset-8/enable && wget -O ./tcp_react_rc2.c https://github.com/ChasLui/Linux-NetSpeed/raw/master/tcp_react_rc2.c && echo "obj-m:=tcp_react_rc2.o" > Makefile && make -C /lib/modules/$(uname -r)/build M=`pwd`
		insmod /root/tcp/tcp_react_rc2.ko
		sysctl -w net.ipv4.tcp_congestion_control=react_rc2
	fi
	cd
