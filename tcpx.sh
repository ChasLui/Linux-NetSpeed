...
Update_Shell() {
  ...
  local shell_url="https://raw.githubusercontent.com/ChasLui/Linux-NetSpeed/master/tcpx.sh"
...
gototcp() {
  ...
  bash <(wget -qO- https://github.com/ChasLui/Linux-NetSpeed/raw/master/tcp.sh)
}
...
# Alpine Linux BBRplus new 安装函数
installbbrplusnew_alpine() {
  echo -e "${Info} Alpine Linux BBRplusNew 安装开始..."
  echo -e "${Tip} 注意: Alpine Linux 将使用标准内核配置BBR，不支持BBRplusNew内核"
  ...
  # 配置 BBR（BBRplusNew在Alpine上退化为BBR）
  configure_bbr_alpine
  
  echo -e "${Tip} Alpine Linux BBR 配置完成（注意：实际为BBR，不是BBRplusNew）"
  read -p "需要重启系统后才能生效，是否现在重启? [Y/n]: " yn
  [ -z "${yn}" ] && yn="y"
  if [[ $yn == [Yy] ]]; then
    echo -e "${Info} 系统重启中..."
...
# Alpine Linux xanmod 安装函数
installxanmod_alpine() {
  echo -e "${Info} Alpine Linux xanmod 安装开始..."
  echo -e "${Tip} 注意: Alpine Linux 将使用标准内核配置BBR，不支持xanmod内核"
  ...
  # 配置 BBR（xanmod在Alpine上退化为BBR）
  configure_bbr_alpine
  
  echo -e "${Tip} Alpine Linux BBR 配置完成（注意：实际为BBR，不是xanmod）"
  read -p "需要重启系统后才能生效，是否现在重启? [Y/n]: " yn
  [ -z "${yn}" ] && yn="y"
  if [[ $yn == [Yy] ]]; then
    echo -e "${Info} 系统重启中..."
...
check_sys_bbrplusnew() {
  ...
  if [[ "${OS_type}" == "CentOS" ]]; then
    ...
    if [[ $(echo "${version}" | cut -d . -f 1) -ge 3 && $(echo "${version}" | cut -d . -f 2) -ge 20 ]]; then
      ...
      echo -e "${Error} BBRplusNew内核不支持当前系统 ${release} ${version} ${bit} ! 需要 Alpine 3.20+" && exit 1
    fi
  else
    echo -e "${Error} BBRplusNew内核不支持当前系统 ${release} ${version} ${bit} !" && exit 1
  fi
...
check_sys_xanmod() {
  ...
  if [[ "${OS_type}" == "CentOS" ]]; then
    ...
    if [[ $(echo "${version}" | cut -d . -f 1) -ge 3 && $(echo "${version}" | cut -d . -f 2) -ge 20 ]]; then
      ...
      echo -e "${Error} xanmod内核不支持当前系统 ${release} ${version} ${bit} ! 需要 Alpine 3.20+" && exit 1
    fi
  else
    echo -e "${Error} xanmod内核不支持当前系统 ${release} ${version} ${bit} !" && exit 1
  fi
